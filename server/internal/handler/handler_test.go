...
func TestDaemonEnrollReturnsDaemonToken(t *testing.T) {
	w := httptest.NewRecorder()
	req := httptest.NewRequest("POST", "/api/daemon/workspaces/"+testWorkspaceID+"/enroll", bytes.NewBufferString(`{"daemon_id":"enrolled-daemon"}`))
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("X-User-ID", testUserID)
	req = withURLParam(req, "workspaceId", testWorkspaceID)

	testHandler.DaemonEnroll(w, req)
	if w.Code != http.StatusOK {
	...
	if !strings.HasPrefix(resp.Token, "mdt_") {
		t.Fatalf("DaemonEnroll: expected mdt_ token, got %q", resp.Token)
	}

	dt, err := testHandler.Queries.GetDaemonTokenByHash(context.Background(), auth.HashToken(resp.Token))
	if err != nil {
		t.Fatalf("DaemonEnroll: expected persisted daemon token: %v", err)
	}
	if uuidToString(dt.WorkspaceID) != testWorkspaceID {
		t.Fatalf("DaemonEnroll: expected workspace %q, got %q", testWorkspaceID, uuidToString(dt.WorkspaceID))
	}
	if dt.DaemonID != "enrolled-daemon" {
		t.Fatalf("DaemonEnroll: expected daemon_id enrolled-daemon, got %q", dt.DaemonID)
	}
	if uuidToString(dt.UserID) != testUserID {
		t.Fatalf("DaemonEnroll: expected user_id %q, got %q", testUserID, uuidToString(dt.UserID))
	}

	testPool.Exec(context.Background(), `DELETE FROM daemon_token WHERE daemon_id = $1`, "enrolled-daemon")
}

func TestDaemonRegisterUsesAuthScopeInsteadOfPayload(t *testing.T) {
	ctx := context.Background()
	...
	if _, err := testHandler.Queries.CreateDaemonToken(ctx, db.CreateDaemonTokenParams{
		...
		DaemonID:    "scoped-daemon",
		UserID:      parseUUID(testUserID),
		ExpiresAt:   pgtype.Timestamptz{Time: time.Now().Add(time.Hour), Valid: true},
	}); err != nil {
		t.Fatalf("CreateDaemonToken: %v", err)
	}
	defer testPool.Exec(ctx, `DELETE FROM daemon_token WHERE daemon_id = $1`, "scoped-daemon")
	defer testPool.Exec(ctx, `DELETE FROM agent_runtime WHERE daemon_id = $1`, "scoped-daemon")
	...
	var gotWorkspaceID, gotDaemonID string
	if err := testPool.QueryRow(ctx, `
		...
		LIMIT 1
	`, "scoped-daemon").Scan(&gotWorkspaceID, &gotDaemonID); err != nil {
		t.Fatalf("DaemonRegister: load runtime: %v", err)
	}
	if gotWorkspaceID != testWorkspaceID {
		t.Fatalf("DaemonRegister: expected workspace %q, got %q", testWorkspaceID, gotWorkspaceID)
	}
	if gotDaemonID != "scoped-daemon" {
		t.Fatalf("DaemonRegister: expected daemon_id scoped-daemon, got %q", gotDaemonID)
	}
}
...
func TestDaemonRegisterUsesEnrollUserAsOwner(t *testing.T) {
	...
	var enrollResp struct { Token string `json:"token"` }
