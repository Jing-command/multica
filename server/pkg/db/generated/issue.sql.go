...
) RETURNING id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, workflow_final_outcome
`
...
SELECT id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, workflow_final_outcome FROM issue
WHERE id = $1
`
...
SELECT i.id, i.workspace_id, i.title, i.description, i.status, i.priority, i.assignee_type, i.assignee_id, i.creator_type, i.creator_id, i.parent_issue_id, i.acceptance_criteria, i.context_refs, i.position, i.due_date, i.created_at, i.updated_at, i.number, i.workflow_final_outcome,
...
const updateIssueWorkflowFinalOutcome = `-- name: UpdateIssueWorkflowFinalOutcome :one
UPDATE issue SET
...
type UpdateIssueWorkflowFinalOutcomeParams struct {
	ID                   pgtype.UUID `json:"id"`
	WorkflowFinalOutcome pgtype.Text `json:"workflow_final_outcome"`
}

func (q *Queries) UpdateIssueWorkflowFinalOutcome(ctx context.Context, arg UpdateIssueWorkflowFinalOutcomeParams) (Issue, error) {
	row := q.db.QueryRow(ctx, updateIssueWorkflowFinalOutcome, arg.ID, arg.WorkflowFinalOutcome)
	var i Issue
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
